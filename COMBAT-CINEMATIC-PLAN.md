...
- The panel sits level with the Menu button, 92px in from the right: the button's 8px inset, its 75.1px width in Cinzel rounded up to 76, then 8 between. The first cut stacked it under the button, and on a 390px-wide phone it landed on the keep, covering the Sniper and Garrick when either was tapped. Level with the button it sits over the ground past the board's far edge at the opening pose.
