...
- The board stands in a lowland: one lit disk at y -1.25, radius 72, out under both rings. Each edge wall drops 1.6 below its own tile, so the walls bottom out between -1.95 under the river and -1.3 under the hills; -1.25 is just over the highest, and any lower would open a slot under the board. It replaced two tries at colouring the dome under the horizon. Flat 0x9fc3d8 there read as an island floating in sky, and in a cut-in the world visibly ended past the board's far edge, where the dome showed between the board and the ridges. Stepping the dome down into green bands read as a backdrop: flat mint round the board at the pose, and bands arcing round the camera at the lowest orbit. Real ground runs from the board's rim to the ridges from every angle.
- The lowland's haze is pinned at 0.2 everywhere, through a tileFog anchor 1000 units out and its own `uHazeMax`. Graded by tile, it stepped in tile-sized squares beside the board and read as more board. At the board's full 0.35 the blue haze turned it teal, (114,143,137) on screen and lighter than the board's front rows; at 0.2 it shows (106,135,107). It takes the board's shadow but no cloud shadow: over a flat plain this size the clouds were blotches all round the board. It stays on the outline layer for the depth edge along the board's rim.
...
- Values after two rounds of screenshots. Sky edges 2, 5, 9 and 14 degrees; the first cut's 3, 8, 15 and 24 hung the top band down over the middle of the frame like a cap, since a rectilinear lens bends rings of constant elevation up at the sides. Zenith 0x5b8ec4, bands evenly mixed in sRGB. Lowland 0x6b8f4a, haze 0.2. Ridge lit 0x7d9a86, shade 0x4c6470, base at -1.75, shoulder 0.45 of the way up. Near ring: 48 columns, ridge 1.8 to 4.4 high, wandering 2, base 5 in from the ridge line, mist 0.15 (first cut 0.3). Far ring: 40 columns, 4.0 to 8.6 high, wandering 2.7, base 8 in, mist 0.45 (first cut 0.6, when it sat within one level of the sky band behind it and read as haze). From a cut-in eye line near 1.0, near peaks rise about 1 to 4 degrees over the horizon and far ones 3 to 7.
- Still to judge: the lowland's green and haze, which are starting values. The board's own far rows, hazed teal by Session 10's 0.35, now sit right against the greener lowland. The waterfalls pour onto the lowland and stop in a hard line, with no pool or splash.
