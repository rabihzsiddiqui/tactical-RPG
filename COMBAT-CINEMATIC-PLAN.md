...
- A dark rim, 3px in the 112px render (1.5 css px in the panel), is traced round each figure in JS after the readback, by the user's call: Kaelen's blue tunic ran into the blue behind his face, and the red-clad enemies into red. Every clear pixel within 3px of the figure takes near-C.table at alpha 235. The render has no antialiasing, so the figure mask is just alpha above 0, read before any rim is painted. Once per portrait at mount, no pass and no toggle.
- Players face screen right and enemies screen left, by the user's call, so the forecast's two faces look at each other. The turn is the same 0.45 rad either way, and the key and rim mirror in x with it, so the lit side stays on the face; a flipped image would have lit the back of the head. Facing is in the cache key. Palettes are already per team, so it is still eleven renders.
...
**Battle forecast (outside the sessions).**
- First cut: the forecast took the unit panel's top-left slot. The user then asked for it near the enemy instead, so Attack is a short reach from the tap that opened it. scene.js places it every frame again: centred over the enemy, above the pair when it fits and below them (under their health bars) when it does not, else on the roomier side, never in the 92px column down the right where the Menu and zoom buttons sit. On a 336px map that column pins it to the left edge. App.jsx still hides the unit panel while it is up, since it shows both faces. It is in the panel's dress, and its Attack and Back are RuleBtn, the Menu button's.
- Layout: the two faces on the outer edges, the numbers between them, four rows of 14px so they stand exactly a face tall. 256px wide. On a 360px phone the map is 336 and the panel gets 236, which leaves the middle column about 27px, so the attacks row is labelled Atks, not Attacks: seven letters did not fit beside two 56px faces. Hit 100 still fits either side at that width.
...
- Back fades it out on the last pair it showed. A new pair is told apart by its unit objects, not the forecast object, which App.jsx builds fresh on every render. With 44px buttons it measures 236 by 200 on a 336px map.

**Battle HUD in the forecast's dress (outside the sessions).**
- The cut-in HUD is the forecast in a wider cut, at the old parchment HUD's size, by the user's call: the full width to 520px and 101px tall (the old one was about 103). To fit that height, each side's HP rides in the names row either side of an HP label that sits over the numbers' labels, and its bar lies along the foot of the face. Names are a size down from the forecast's (12px), so "Mercenary" and 21/21 share a side of a 320px HUD. Weapons show beside the names from 420px of content width, a container query on the HUD itself; the weapon takes only what the name and HP leave, so it is what truncates.
- Attack grows the HUD out of the forecast: the forecast hands its box to `morph.js` and drops out with no fade, and the HUD, mounting in the same React render (doAttack reaches `g.cutIn` before its first await), animates its frame from that box to its own by transform (240ms), then fades its contents in from 150ms. The contents never scale. The box goes stale after 600ms, so Attack with the cut-in off, which hands one over too, never reaches a later HUD. The flip clears its transform before measuring, or StrictMode's second effect run measured the first run's flight. Enemy-phase and heal HUDs rise in as before.

**Action menu in the same dress (outside the sessions).**
- Attack (or Heal), Vulnerary, Wait and Back are now a dark panel with the gold rule, commands in Cinzel, and the pause menu's gold bar drawn on the hovered or focused row rather than sliding between rows. Rows are 40px tall and the panel sizes to "Vulnerary", 139 by 176px. scene.js places it by its measured size: right of the unit, or left of it when the right side would run into the 92px button column, which on a 362px phone map is any unit right of about a third of the way across. `Item` in primitives.jsx had no other user and went with the old menu.
...
**Text face (outside the sessions).**
- Every label, number and sentence is now Spectral, self-hosted like Cinzel (Google Fonts' Latin subset, 400 and 600, about 15KB each), with `lining-nums tabular-nums` on the body. It replaced the monospace, which the user said read as techy, and Georgia too: two serifs would have met in one card (the Orders paragraph over its manual button), and Android has no Georgia, so the body was a different face on the OnePlus anyway. The MONO token is gone; SERIF is Spectral first.
- Picked from a side-by-side in the game's own roles (stat grid, forecast row, hint line, log, triangle card, pills) at the sizes the monospace had. Alegreya, EB Garamond and Crimson Pro set noticeably smaller at the same px and would have needed every size bumped; Lora was the most legible but heavier; Marcellus pairs with Cinzel but its 1 and 0 read as I and O. Spectral held the sizes and its capitals sit near Cinzel's. No size was changed.

**Rising numbers (outside the sessions).**
- Damage and heal numbers are `ui/Floaters.jsx` now: Cinzel figures, 28px (36 for a crit, 18 for a miss, up from 15 for all), light with a dark outline, on a burst drawn as the div's ::before. scene.js's `floater()` passes a kind (hit, crit, heal, miss) instead of a colour, which left scene.js with no import from the ui theme. The bursts use two colours the scene already had, added to C as `hurt` (the enemy health bar's red, 0xd94f45) and `mend` (the staff heal's tint, 0x5fd07a): the theme's own green was near enough the grass that a heal's aura vanished on it, and C.red read muddy over the sky. A near-solid core to 25% then a soft edge; a soft ramp is fine here, since the numbers are html above the posteriser. A pop from 0.7 to 1.1 to 1, then the old 900ms climb and fade; fade only under reduced motion.

**Enemy reds (outside the sessions).**
- The foe and boss tunics, capes and plumes are far more saturated than before (foe tunic 0x9d3339 to 0xc4222b), by the user's call that enemies read too dull on the map. A first step to 0xb1262d moved the rendered tunic's red channel at the reference pose only from 38.7% to 41.3%: the haze and the cool ambient hold green and blue at a floor (22.4% and 23.5%, unchanged by either step), so only more red reads as redder. The second step reached 46.3%. In the cut-in it is a strong red but not a hot one.

**Northern range and volcano (outside the sessions).**
- By the user's call, a volcanic range wraps the board's northern half in `sky.js` (`buildRange`): from just north of the river up the west side, round the north edge and down the east side, so the river still runs out both ways. Its front is the board's edge; each column fans its rows out along the edge's normal, round the corners too, and each flank falls to the ground over its last 3 units at the river. Seven rows: an apron of settled ash to 2.3 out, then the foot, foothills, shoulders, upper slopes and a ridge line up to 6. One mesh, flat colour per face baked from the sun like the ridges, noOutline, fogged by distance. The volcano stands behind the ridge left of the keep. It first had its rim at 10, a lava pool and three glowing streaks down its south face; the user found the streaks looked bad and wanted an active volcano that only smokes, so the streaks and the pool are gone, the crater floor is the crater wall's dark, and the rim is down to 7.6, still clear of the ridge line's 6.
- The lens: the cut-in camera never goes more than about 2.3 from the pair's midpoint, so nothing but the apron (under 0.22) is ever within 2.3 of the board's edge, and `groundAt` needs no knowledge of the range.
- What shows where: the home view's frame top sits about 33 degrees down, so past the north edge it sees only the apron and the foot of the slopes (about 3.4 of height fits at the edge, 2 at 7 out). A wide desktop sees the flanks beside the board in full. Peaks, the volcano and its smoke are cut-in and zoomed-in scenery.
- The fog round the map used to skip anything standing over 0.05 within 10 of the edge (canopies and ash overhang it), then jump to about 31%. A slope crossing that line would have shown the jump, so it is now a ramp from 1 to 20, applied after the quantiser like the rest of the fog. Nothing stood in that band before. The first version at 10 left the near range pale; 20 keeps it close to its own colour.
- The smoke is `buildPlume` in `wind.js`, 56 hard-edged discs on PLUME_VERT sharing the ash's uWind and uTime, sized in world units by the target height read in onBeforeRender. It is created from `createSky` so the new objects still come last for the seeded draws.
- Ash: 240 flakes over a volume now reaching 5.5 past the north edge and 2.5 past each side, with ASH_NORTH_BIAS 2.0 bending the depth after the wrap so they crowd north and stay crowded as they drift. The board itself holds far fewer than the old 320.
- The northern tiles first took a grey ash tint, 55% down to 12% by row. The user asked for brown near the volcano instead: now a share of each row north of the river (70%, 50%, 30%, 12%, picked by a hash so they lie in patches) goes 70 to 90% toward a burnt brown, 0x7a5232, and the rest keep a lighter grey dusting (30% down to 5%) that runs on into the apron. Stonework, the keep and the ridge at height 0.6, never scorches. A first brown at 50 to 78% read as olive through the tile haze.
- Starting values, tuned once from headless renders (the first cut's ridge at 8.6 walled the board in at low angles and read pale; lowered, darkened, finer facets). Not yet judged on a device.

**Ash spill and drifts off the range's flanks.**
- Every row of each flank stopped on one line 1.2 north of the river's north bank. From the home view that line ran straight across the frame on both sides of the board, with rock and ash above it and a strip of green below. The user read it as a harsh line between the green and the brown.
- Found without the image viewer: the harness frame printed as a character map (G green, B brown or grey, S sky or fog, W water) showed rows 0 to 13 brown and row 14 green across the full width.
- `fray` in `buildRange` lays a flat strip of ash south of a line, with a south edge set by smooth noise on SPILL_PATCH 2.5 knots. Patches take the apron's ash or meshes.js's SCORCH, now exported. It sits at SPILL_Y 0.01, under the apron front row's 0.02, and joins the range mesh, so there is no new draw call or pass. It is unfogged near the board, like the apron, because of the alpha gate.
- The spill runs from the flank's end line toward the north bank, from 15 to 98 percent of the gap, out to SPILL_OUT 10 inside the flank's ridge line, so its north edge stays under rock.
- Once the spill reached the bank, the river became the rule, brown north and green south. So drifts cross it: from the south bank, DRIFT_FROM 0.8 to DRIFT_OUT 8 beyond the board's edge, up to DRIFT_MAX 3.5 south, covering DRIFT_COVER 0.45 of the bank with green between them. Both ends taper. The first cut started them at the board's edge and made a new straight line against the board's green tiles.
- Starting values, not judged against screenshots. The reference pose changes only in the band round the river; the reference cut-in moves by 8 pixels.

**Forecast HP pulse and the heal forecast (outside the sessions).**
- Each forecast health bar pulses over the HP the exchange could take from it, by the user's call: the other side's damage times its attacks, crits aside, capped at the HP left, so a bar that pulses to its end is a possible kill. The pulse sits at the fill's inner end, toward the middle where the bars drain. `Hp` in Forecast.jsx takes `loss` and `gain`; the battle HUD shares `Hp` and passes neither, since the exchange is already playing there. Opacity 1 to 0.2 and back over 650ms each way; under reduced motion it holds at 0.5.
- The user asked for the same on "the healing forecast", which did not exist: a heal went off the moment its target was tapped. Healing now opens the forecast in heal mode (healer left, target right, the amount in place of the numbers, the target's bar pulsing green in C.mend over what it would fill, Heal in place of Attack), from the Heal menu's target tap and from tapping an ally straight from a selection. Heal morphs into the heal cut-in as Attack does. Auto battle calls resolveHeal directly and never sees it. The amount is `healAmount` in core/combat.js, which resolveHeal now calls too, so the preview and the heal cannot disagree.
