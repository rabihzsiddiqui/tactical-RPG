...
**Sky and ridges (Session 12).**
- The orbit never looks above the horizon. Its pitch stops at 20 degrees down, where the top edge of the frame is still about 5 degrees under it. So the sky bands only ever show in a cut-in, and their edges are placed for that view, which reaches about 26 degrees up at the top centre. The orbit sees only the dome under the horizon.
...
- The ridges' faces stop at a haze line at y -1, wandering 0.6 either way per column. The first cut carried them down to a foot at -12, and from the lowest orbit they stood behind the board as a wall of facets taller than the board itself. The second painted that lower row in the horizon colour, which hid it against a flat sky but would hide the green ground as well. Now nothing is built under the line and the dome shows through. In a cut-in the line sits about 2 degrees under the horizon, inside the dome's haze band, so the peaks rise out of mist.
- Under the horizon the dome steps from the haze colour down into green (`GROUND`) at 4, 8 and 14 degrees down. Asked for after the first screenshots: the board should read as a plateau over far-off country, not an island floating in sky, with the focus still on the map. This deliberately gives up the brief's grid view unchanged. The green is paler and softer than the board's grass, and at the reference pose everything around the board is the deepest band, one flat colour. The haze band runs from 4 degrees under the horizon to 2 over it, so the strip under the ridges in a cut-in stays pale, which was also asked for. From the lowest orbit the ground bands arc around the camera behind the board.
...
- Values after the first screenshots. Sky edges 2, 5, 9 and 14 degrees; the first cut's 3, 8, 15 and 24 hung the top band down over the middle of the frame like a cap, since a rectilinear lens bends rings of constant elevation up at the sides. Zenith 0x5b8ec4. Ground 0x8aad7e, edges 4, 8 and 14 degrees down. All bands evenly mixed in sRGB. Ridge lit 0x7d9a86, shade 0x4c6470. Near ring: 48 columns, ridge 1.8 to 4.4 high, wandering 2, faces meeting the haze line 1.8 in from the ridge, mist 0.15 (first cut 0.3). Far ring: 40 columns, 4.0 to 8.6 high, wandering 2.7, 3.6 in, mist 0.45 (first cut 0.6, when it sat within one level of the sky band behind it and read as haze). From a cut-in eye line near 1.0, near peaks rise about 1 to 4 degrees over the horizon and far ones 3 to 7.
- Still to judge: the ground green, a starting value, and the ridge strip along the top of the lowest orbit, which reads as a flat grey-green band.
