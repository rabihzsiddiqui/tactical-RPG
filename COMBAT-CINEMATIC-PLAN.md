...
- The orbit never looks above the horizon. Its pitch stops at 20 degrees down, where the top edge of the frame is still about 5 degrees under it. So the dome's bands only ever show in a cut-in, and the band edges (3, 8, 15, 24 degrees) are placed for that view, which reaches about 26 degrees up at the top centre. Under the horizon the dome is exactly 0x9fc3d8.
...
- The haze line. Faces under y -1 (wandering 0.6 either way per column) are the horizon colour exactly. At full colour the lowest orbit looked down onto the rings' inner faces, and they stood behind the board as a wall of facets taller than the board itself. Now the orbit at pitch 20 sees the old flat sky with a thin strip of ridge tops along the top edge, and at 48 sees no ridge at all. In a cut-in, only the peaks rising out of the haze show.
- Ridge colour is baked per face at mount: how squarely the face meets the sun in LIGHTS picks a point between shade and lit, and the ring's mist mixes that toward the horizon, all in sRGB. `MeshBasicMaterial`, so the haze and cloud patches never touch it and no light change moves it. `noOutline`, so the ridges take no lines. The brief asked whether they should; this was the default while the question was open.
- Starting values, before any tuning against screenshots: zenith 0x5b8ec4, bands evenly mixed in sRGB; ridge lit 0x7d9a86, shade 0x4c6470, foot at -12. Near ring: 48 columns, ridge 1.8 to 4.4 high, foot 6.5 inside the ridge line, mist 0.3. Far ring: 40 columns, 4.0 to 8.6 high, foot 9 in, mist 0.6. From a cut-in eye line near 1.0, near peaks rise about 1 to 4 degrees over the horizon and far ones 3 to 7.
- Two things to judge. The band edges are circles of constant elevation, so through the cut-in's 66 degree fov they curve up across the frame, most visibly at 15 and 24 degrees. And where the board does not hide it, the haze under the ridges shows as the flat horizon colour between the board's far edge and the peaks.
- Checked on the seeded, fixed-clock harness at 1000x500: the reference pose and cut-in at 400x240 and native, the orbit at pitch 20 from three yaws, and three real attacks: along z, along x, and a bow shot on a diagonal. Every cut-in frame has both ridge rings on the horizon behind the fighters. No console errors.
