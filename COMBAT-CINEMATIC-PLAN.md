...
- The forecast takes the unit panel's slot, top left at the same 8px inset with the same 92px kept for the Menu button, by the user's call, and App.jsx hides the panel for as long as it is up. It is in the panel's dress, and its Attack and Back are the Menu button's. scene.js no longer places it every frame; it used to float above the two units.
