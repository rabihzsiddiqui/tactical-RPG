...
- The lowland is hazed in rings by distance from the board's edge, not from the camera, so the map is the clear thing in the middle and the land steps back from it: 0.25 from the edge, then 0.4 from 2 units out, 0.55 from 5, 0.7 from 9, 0.85 from 15, and 1 from 24. The distance is to the board's rectangle, so each ring follows its outline, rounded at the corners. 1 is exactly the dome's colour under the horizon, reached well before the near ridges, so the lowland's far reaches and the dome meet with no edge and the far-plane clip of the disk never shows. The earlier tries: graded by tile like the board it stepped in tile squares and read as more board; pinned at the board's 0.35 the blue haze turned it teal and lighter than the board's front rows; pinned at 0.2 it read as out of place. It takes the board's shadow and the cloud shadow again: flush with the board, clouds would otherwise stop dead at the map's edge and draw its rectangle, and the rings fade them out with distance, where at one flat haze they were blotches.
- The river runs on out of the map across the lowland to 44 units from the centre, deep in the full haze, in a channel walled down to the river bed like the banks inside the board. The waterfalls and WATER_FRAG's white lip at the map's edge are gone: with the ground at the map's level there is nothing to fall off. The run-out is WATER_FRAG with RING_HAZE defined, on the board water's own uniform objects so uTime moves with it; out there the shore field reads its clamped edge column, which is the same banks the river had where it left the map. Each strip starts on the board's edge with six cells a unit across, as a water tile has, so the two meet vertex for vertex.
...
- Values after three rounds of screenshots. Sky edges 2, 5, 9 and 14 degrees; the first cut's 3, 8, 15 and 24 hung the top band down over the middle of the frame like a cap, since a rectilinear lens bends rings of constant elevation up at the sides. Zenith 0x5b8ec4, bands evenly mixed in sRGB. Lowland 0x6b8f4a, haze rings as above, tuck 0.25. River run-out to 44, surface at -0.1 as on the board, 2 cells a unit along it. Ridge lit 0x7d9a86, shade 0x4c6470, base 0.5 under the lowland, shoulder 0.45 of the way up. Near ring: 48 columns, ridge 1.8 to 4.4 high, wandering 2, base 5 in from the ridge line, mist 0.15 (first cut 0.3). Far ring: 40 columns, 4.0 to 8.6 high, wandering 2.7, base 8 in, mist 0.45 (first cut 0.6, when it sat within one level of the sky band behind it and read as haze). From a cut-in eye line near 1.0, near peaks rise about 1 to 4 degrees over the horizon and far ones 3 to 7.
- Still to judge: the ring amounts and spacing, which are starting values. The river run-out stays bright through the haze, since water starts far lighter than grass: at the 0.7 ring it shows about (118,175,191) beside grass at (130,167,160), a bright line leading out of the map. The board's own far rows, hazed teal by Session 10's 0.35, sit next to the first ring's 0.25.
